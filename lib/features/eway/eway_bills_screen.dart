...
class _EWayBillsScreenState extends State<EWayBillsScreen> {
                  ...
                  try {
                    await widget.api.post(
                      ...
                      body: {
                        ...
                        'trans_doc_number': docNo.text.trim().isEmpty
                            ? null
                            : docNo.text.trim(),
                        'trans_doc_date': docDate == null
                            ? null
                            : apiDate(docDate!),
                        'trans_distance': km,
                        'trans_mode': mode,
  ...
  Widget build(BuildContext context) => PageFrame(
    title: 'E-Way Bills',
    subtitle: 'Transport compliance for qualifying movement of goods, with vehicle history, invoice/bill source and consolidation.',
    actions: [
    ...
    child: _loading
        ...
        ? const EmptyState(
            ...
            message: 'Generate an e-Way Bill for eligible finalized goods invoices or bills.',
          )
        : Column(
            children: _items
                .map(
                  (e) => Padding(
                    ...
                    child: Card(
                      child: ListTile(
                        ...
                        subtitle: Text(
                          '${titleCase('${e['supply_type'] ?? ''}')} • ${e['invoice_id'] != null
                              ? 'Invoice'
                              : e['bill_id'] != null
                              ? 'Bill'
                              : 'Document'} • ${e['vehicle_number'] ?? ''} • ${e['trans_distance'] ?? 0} km\nValid until ${displayDate(e['valid_until'])}',
                        ),
                        isThreeLine: true,
