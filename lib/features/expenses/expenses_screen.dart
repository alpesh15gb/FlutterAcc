...
class _ExpensesScreenState extends State<ExpensesScreen> {
  ...
  Future<void> _clone(String id) async {
    try {
      ...
      showMessage(
        context,
        'Expense cloned as ${clone['expense_number'] ?? 'draft'}.',
      );
      await _load();
      if (!mounted) return;
      final saved = await Navigator.push<bool>(
  ...
  Widget build(BuildContext context) => PageFrame(
    title: 'Expenses',
    subtitle: 'Operating spend with input GST, categories and auditable ledger reversals.',
    actions: [
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (mounted) {
        showMessage(
          context,
          _editing ? 'Draft expense updated.' : 'Expense posted.',
        );
                    ...
                    SectionCard(
                      ...
                      child: _preview == null
                          ...
                          : Wrap(
                              ...
                              runSpacing: 12,
                              children: [
                                _previewMetric('Taxable', _preview!['amount']),
                                _previewMetric(
                                  'CGST',
                                  _preview!['cgst_amount'],
                                ),
                                _previewMetric(
                                  'SGST',
                                  _preview!['sgst_amount'],
                                ),
                                _previewMetric(
                                  'IGST',
                                  _preview!['igst_amount'],
                                ),
    ...
    content: SizedBox(
      ...
      child: _loading
          ...
          ? const EmptyState(
              ...
              title: 'No categories',
              message: 'Create a category and link it to a ledger account.',
            )
          : ListView.separated(
              ...
              itemBuilder: (_, i) {
                ...
                final linked = _accounts
                    .where((a) => '${a['id']}' == '${c['linked_account_id']}')
                    .toList();
                return ListTile(
                  title: Text(
                    '${c['name'] ?? ''}',
    ...
    try {
      ...
      if (widget.row == null) {
        ...
        await widget.api.put(
          '/masters/expense-categories/${widget.row!['id']}',
          body: body,
        );
      }
      ...
      child: Column(
        ...
        children: [
          ...
          DropdownButtonFormField<String?>(
            ...
            isExpanded: true,
            decoration: const InputDecoration(
              labelText: 'Linked ledger account',
            ),
            items: [
              const DropdownMenuItem<String?>(
                value: null,
