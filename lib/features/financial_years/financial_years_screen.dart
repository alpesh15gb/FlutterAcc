...
class _FinancialYearsScreenState extends State<FinancialYearsScreen> {
  ...
  Future<void> _create() async {
    ...
    final name = TextEditingController(
        text:
            'FY ${start.year}-${(end.year % 100).toString().padLeft(2, '0')}');
            ...
            FilledButton(
              onPressed: () async {
                if (name.text.trim().isEmpty || end.isBefore(start)) {
                  showMessage(context, 'Enter a valid name and date range.',
                      error: true);
                  return;
                }
                try {
                  await widget.api.post('/financial-years', body: {
                    'name': name.text.trim(),
      ...
      await showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: SizedBox(
              ...
              height: 560,
              child:
                  SingleChildScrollView(child: JsonReportView(data: report))),
          actions: [
            TextButton(
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              try {
                await widget.api.post('/financial-years/${fy['id']}/reopen',
  ...
  Future<void> _lockPeriod(
      {required bool lock, Map<String, dynamic>? existing}) async {
  ...
  Widget build(BuildContext context) => PageFrame(
        ...
        subtitle:
            'Switch books, inspect close readiness, reopen closed years and lock monthly posting periods.',
        actions: [
