...
class _ImportToolsScreenState extends State<ImportToolsScreen> {
  ...
  Future<void> _vyapar() async {
    ...
    if (file == null) return;
    await _run('Vyapar migration result',
  ...
  Future<void> _tally() async {
    ...
    if (file == null) return;
    await _run(
        'Tally import result', () => widget.api.upload('/tally/import', file));
  }
  ...
  Future<void> _csvDryRun() async {
    ...
    await _run('CSV validation report', () async {
      final data = await widget.api
          .upload('/import/csv', file, query: {'dry_run': 'true'});
      _csvValidation = data;
      return data;
    });
  }
  ...
  Future<void> _csvCommit() async {
    ...
    await _run(
        'CSV import result',
        () => widget.api
            .upload('/import/csv', file, query: {'dry_run': 'false'}));
  }
  ...
  Widget build(BuildContext context) => PageFrame(
      ...
      subtitle:
          'Move existing Indian accounting data into ApexBooks with validation-first workflows.',
      child: Column(children: [
        ...
        if (_loading)
          ...
          SectionCard(title: _resultTitle, child: JsonReportView(data: _result))
        else
          const SectionCard(
              child: EmptyState(
                  ...
                  message:
                      'Imports are executed by the FastAPI backend so posting, stock and GST integrity stay consistent.'))
      ]));
...
class _ToolCardData {
  const _ToolCardData(
      this.title, this.subtitle, this.icon, this.onTap, this.action);
