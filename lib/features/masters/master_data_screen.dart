...
class _MasterDataScreenState extends State<MasterDataScreen>
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              DropdownButtonFormField<String?>(
                ...
                items: [
                  ...
                  ..._accounts.where((a) => a['is_active'] != false).map(
            ...
            FilledButton(
              onPressed: () {
                if (name.text.trim().isEmpty) {
                  showMessage(context, 'Category name is required.',
                      error: true);
                  return;
                }
            ...
            FilledButton(
              onPressed: () {
                ...
                if (name.text.trim().isEmpty ||
                    ...
                    parsed > 100) {
                  showMessage(
                      context, 'Enter a name and GST rate from 0 to 100.',
                      error: true);
                  return;
                }
                Navigator.pop(dialogContext, {
  ...
  Future<void> _deleteTax(Map<String, dynamic> tax) async {
    ...
    final ok = await _confirm(
        'Deactivate GST rate?', 'Deactivate ${tax['name']} (${tax['rate']}%)?');
    if (!ok) return;
    try {
  ...
  Widget build(BuildContext context) => PageFrame(
        title: 'Master Data',
        subtitle:
            'Expense categories, GST rate templates and payment terms used across transactions.',
        actions: [
  ...
  Widget _categoriesView() {
    if (_categories.isEmpty) {
      return EmptyState(
        ...
        message:
            'Create categories and link them to ledger accounts for expense posting.',
        ...
        child: DataTable(
          ...
          rows: _categories.map((c) {
            ...
            return DataRow(cells: [
              ...
              DataCell(SizedBox(
                  width: 260, child: Text('${c['description'] ?? '—'}'))),
              DataCell(Text(linked == null
                  ? 'Backend default'
                  : '${linked['code']} • ${linked['name']}')),
              DataCell(Text(c['is_active'] == false ? 'INACTIVE' : 'ACTIVE',
                  style: TextStyle(
      ...
      child: Column(
        children: _terms
            .map((t) => ListTile(
                  ...
                  title: Text('${t['name'] ?? ''}',
                      style: const TextStyle(fontWeight: FontWeight.w700)),
                  subtitle: Text(t['tenant_id'] == null
                      ? 'System standard'
                      : 'Company term'),
                  trailing: Text('${t['due_days'] ?? 0} day(s)',
