  ...
  Future<void> _cancel(Map<String, dynamic> row) async {
    ...
    try {
      await widget.api.post(
        '$endpoint/${row['id']}/cancel',
        body: {'reason': note, 'cancellation_date': apiDate(date)},
      );
      if (mounted)
        showMessage(
          context,
  ...
  Widget build(BuildContext context) => PageFrame(
    ...
    actions: [
      ...
      FilledButton.icon(
        onPressed: _create,
        icon: Icon(
          widget.vendor ? Icons.north_east_rounded : Icons.south_west_rounded,
        ),
...
class _PaymentEditorScreenState extends State<PaymentEditorScreen> {
      ...
      if (mounted)
        setState(() {
          ...
          for (final row in _outstanding) {
            _allocations[row['id'].toString()] = TextEditingController(
              text: '0',
            );
          }
        });
  ...
  Future<void> _save() async {
    ...
    if (_contactId == null || amount <= 0) {
      showMessage(
        context,
        'Select a party and enter a positive payment amount.',
        error: true,
      );
      return;
    }
    ...
    for (final row in _outstanding) {
      ...
      if (v > 0)
        allocations.add({
          widget.vendor ? 'bill_id' : 'invoice_id': row['id'],
          'amount': v,
        });
    }
    ...
    try {
      final body = <String, dynamic>{
        ...
        'payment_number': _number.text.trim().isEmpty
            ? null
            : _number.text.trim(),
      ...
      if (mounted) {
        showMessage(
          context,
          widget.vendor ? 'Vendor payment posted.' : 'Customer receipt posted.',
        );
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(
      title: Text(
        widget.vendor ? 'Record vendor payment' : 'Record customer receipt',
      ),
      actions: [
        Padding(
