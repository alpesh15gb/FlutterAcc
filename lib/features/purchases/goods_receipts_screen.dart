...
class _GoodsReceiptsScreenState extends State<GoodsReceiptsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      _items = _rows(
        await widget.api.get('/goods-receipts', query: {'limit': 100}),
      );
    } catch (e) {
      _error = e.toString();
    }
  ...
  Future<void> _create() async {
    try {
      ...
      if (poRows.isEmpty) {
        showMessage(
          context,
          'Create a purchase order before receiving goods.',
          error: true,
        );
        return;
      }
  ...
  Widget build(BuildContext context) => PageFrame(
    ...
    subtitle: 'Receive items against purchase orders and update warehouse stock only after confirmation.',
    actions: [
                    ...
                    trailing: status == 'DRAFT'
                        ? PopupMenuButton<String>(
                            ...
                            itemBuilder: (_) => const [
                              PopupMenuItem(
                                value: 'confirm',
                                child: Text('Confirm & receive stock'),
                              ),
                              PopupMenuItem(
                                value: 'cancel',
                                child: Text('Cancel'),
          ...
          'lines': active
              .map(
                (l) => {
                  ...
                  'lot_number': l.lot.text.trim().isEmpty
                      ? null
                      : l.lot.text.trim(),
                  'batch_number': l.batch.text.trim().isEmpty
                      ? null
                      : l.batch.text.trim(),
                },
              )
