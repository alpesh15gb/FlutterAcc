      ...
      final saved = await Navigator.push<bool>(
        ...
        MaterialPageRoute(
            builder: (_) => _ReturnEditor(
                api: widget.api, purchase: widget.purchase, documents: docs)),
      );
      if (saved == true) _load();
    } catch (e) {
            ...
            : _error != null
                ...
                : _items.isEmpty
                    ? EmptyState(
                        ...
                        title: 'No returns',
                        message:
                            'Returns created from posted source documents will appear here.',
...
class _ReturnEditor extends StatefulWidget {
  const _ReturnEditor(
      {required this.api, required this.purchase, required this.documents});
  final ApiClient api;
  final bool purchase;
...
class _ReturnEditorState extends State<_ReturnEditor> {
  ...
  Future<void> _loadDetail() async {
    ...
    try {
      final path =
          widget.purchase ? '/bills/$_documentId' : '/invoices/$_documentId';
  ...
  Future<void> _save() async {
    ...
    final chosen = _lines
        ...
        .toList();
    if (chosen.isEmpty) {
      showMessage(context, 'Enter a return quantity for at least one line.',
          error: true);
      return;
    }
