...
class _InvoicesScreenState extends State<InvoicesScreen> {
    ...
    final saved = await Navigator.push<bool>(
      ...
      MaterialPageRoute(
          builder: (_) => TaxDocumentEditorScreen(
              api: widget.api, kindName: 'invoice', initialId: id)),
    );
    if (saved == true) _load();
  }
        ...
        actions: [
          ...
          if ((status == 'POSTED' ||
            ...
            TextButton.icon(
              ...
              label: Text(eInvoiceStatus == 'FAILED'
                  ? 'Retry e-invoice'
                  : 'Generate IRN'),
            ),
  ...
  Widget _invoiceDetail(Map<String, dynamic> data) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _detailRow(
          'GST',
          money((num.tryParse('${data['cgst_amount'] ?? 0}') ?? 0) +
  ...
  Future<void> _mutate(String id, String suffix, String label,
  ...
  Future<void> _cancelEInvoice(String id) async {
    ...
    try {
      await widget.api.post('/invoices/$id/e-invoice/cancel', body: {
  ...
  Future<void> _downloadPdf(Map<String, dynamic> invoice) async {
    ...
    try {
      ...
      final number = invoice['invoice_number']
              ?.toString()
  ...
  Widget _statsBar() => LayoutBuilder(builder: (context, c) {
        final cards = [
          (
            'Total',
            _stats['total_amount'],
          ...
          child: DataTable(
            ...
            rows: _items
                .map((i) => DataRow(
                      ...
                      cells: [
                        DataCell(Text(i['invoice_number']?.toString() ?? '',
                            style:
          ...
          child: InkWell(
            ...
            child: Padding(
              ...
              child: Column(children: [
                Row(children: [
                  Expanded(
                      child: Text(i['invoice_number']?.toString() ?? '',
                          style: const TextStyle(fontWeight: FontWeight.w900))),
                ...
                Row(children: [
                  Expanded(
                      child: Text(i['contact_name']?.toString() ?? '',
