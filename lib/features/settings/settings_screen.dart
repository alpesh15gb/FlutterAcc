...
class _SettingsScreenState extends State<SettingsScreen>
  ...
  Widget build(BuildContext context) => PageFrame(
    title: 'Settings',
    subtitle: 'Company, branches, GST credentials, numbering, terms, team, security and backup.',
    actions: [
  ...
  Widget _companyTab() {
    final legal = TextEditingController(
      text: _company['legal_name']?.toString() ?? '',
    );
    final trade = TextEditingController(
      text: _company['trade_name']?.toString() ?? '',
    );
    final gstin = TextEditingController(
      text: _company['gstin']?.toString() ?? '',
    );
            ...
            builder: (context, setLocal) => Column(
              ...
              children: [
                ...
                FilledButton.icon(
                  onPressed: () async {
                    try {
                      final data = await widget.api.put(
                        '/companies/$_tenantId',
                        body: {
                          'legal_name': legal.text.trim(),
                          'trade_name': trade.text.trim().isEmpty
                              ? null
                              : trade.text.trim(),
  ...
  Future<void> _editBranch([Map<String, dynamic>? existing]) async {
    ...
    final stateCode = TextEditingController(
      text: address['state_code']?.toString() ?? '',
    );
          ...
          FilledButton(
            onPressed: () async {
              if ([
                ...
                showMessage(
                  context,
                  'Name and a complete Indian address are required.',
                  error: true,
                );
                return;
              }
  ...
  Future<void> _toggleBranch(Map<String, dynamic> branch) async {
    try {
      await widget.api.put(
        '/companies/$_tenantId/branches/${branch['id']}',
  ...
  Widget _taxTab() {
    ...
    final upi = TextEditingController(
      text: _settings['upi_id']?.toString() ?? '',
    );
              ...
              onPressed: () async {
                try {
                  final body = <String, dynamic>{
                    ...
                    'e_invoice_username': einUser.text.trim().isEmpty
                        ? null
                        : einUser.text.trim(),
  ...
  Future<void> _editSeries(Map<String, dynamic> s) async {
    ...
    final next = TextEditingController(
      text: s['next_number']?.toString() ?? '1',
    );
    final suffix = TextEditingController(text: s['suffix']?.toString() ?? '');
    final padding = TextEditingController(
      text: s['padding_digits']?.toString() ?? '4',
    );
    final saved = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
                ...
                .map(
                  (t) => ListTile(
                    ...
                    subtitle: Text(
                      t['is_preset'] == true
                          ? 'Preset'
                          : (t['is_active'] == false ? 'Inactive' : 'Custom'),
                    ),
                ...
                .map(
                  (m) => ListTile(
                    ...
                    trailing: Wrap(
                      ...
                      children: [
                        Chip(label: Text(m['role']?.toString() ?? 'member')),
                        PopupMenuButton<String>(
                          onSelected: (v) {
  ...
  Widget _securityTab() => Column(
    children: [
      SectionCard(
        title: 'Password',
        child: _ChangePassword(api: widget.api),
      ),
      const SizedBox(height: 14),
      SectionCard(
